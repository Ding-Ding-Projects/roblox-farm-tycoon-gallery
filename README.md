...
Public gallery of 59 genuine reviewed captures. Newest dated evidence appears first using validated receipt timestamps, with unavailable dates last. Every image has a SHA-256 entry in [the public inventory](capture-inventory.json).
...
The 59-image source update passes all local hash/image checks. Hosted delivery of this update is pending; the earlier 51-image delivery receipt remains historical evidence. Diagnostic probes, verbose planting views pending readability review, and duplicate reward framing are excluded with reasons in the inventory.
