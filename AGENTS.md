...
This repository contains only the public static capture gallery. Keep original image bytes, verify each image hash against `capture-inventory.json`, and inspect pixels and metadata before publication. Capture timestamps must come from validated source receipts. Label development observations and defects honestly. Never copy private source, profile records, account imagery, editor surfaces or local machine paths into this repository.
